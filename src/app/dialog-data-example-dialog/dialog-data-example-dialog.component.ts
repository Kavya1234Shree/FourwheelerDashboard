...
if(event.target.checked)
{
  ...
  ngOnInit() {
   let resp = this.http.get<any>("https://apis.zeigerapp.in/api/driverfour-owner/phone?phone="+this.phone);
  ...
  onDriver(driver_name,email,phone,driver_address,driver_city,driver_state,driver_zip,alternate_contact_num,
   ...
   body_type,will_accept_to_work_for_below_category,bank_name,account_no,IFSC_code,type_of_account,owner_name,owner_phone,address1,city,state,zip,active){
    ...
    if( this.driver_name==""){
      ...
      // else if(this.user!==this.user && this.phone!==phone){
      //    alert("cannot upadat");
      // }
                  ...
                  this.closeModal()
                      this.http.post('https://zeigerapp.in/api/1.0/driver-owner/phone?phone='+this.phone,fd)
